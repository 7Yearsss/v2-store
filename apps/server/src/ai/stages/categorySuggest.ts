...
/**
 * categorySuggest stage：遇到未见过的来源类目时跑一次。
...
async function fewShotExamples(db: Db, workspaceId: string, channel: string) {
  const rows = await db
    .select({
...
async function saveSuggestion(
  ctx: StageContext,
  item: Pick<SourceItemRow, "sourceCategoryId" | "sourceCategoryName">,
  ...
  await replaceStageSuggestions(ctx.deps.db, ctx.listing, categorySuggestStage, [
    { field: "category", value },
  ]);
}

export const categorySuggestStage: Stage = {
  key: "categorySuggest",
  kind: "llm",
  fields: ["category"],
  async run(ctx) {
    const { deps, listing, store, item } = ctx;
    ...
    const meta = { workspaceId: listing.workspaceId, listingId: listing.id };
    ...
    if (adapter.predictCategories) {
      ...
      const candidates = (
        await adapter.predictCategories(deps, store, {
          title: query,
          sourceCategoryName: item.sourceCategoryName,
      ...
      await saveSuggestion(ctx, item, candidates);
      return;
    }
    ...
    await saveSuggestion(ctx, item, final);
  },
};
