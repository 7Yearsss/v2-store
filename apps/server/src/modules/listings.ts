...
export function toListingDto(r: ListingRow): Listing {
  return {
    id: r.id,
    storeId: r.storeId,
...
const patchSchema = z
  .object({
    ...
    images: z.array(z.string().url()).max(250),
    options: z
...
export function listingRoutes() {
  ...
  r.get("/", zValidator("query", listQuery), async (c) => {
    ...
    return c.json({ items: rows.map(toListingDto), total: total?.n ?? 0 });
  });
  ...
  r.get("/:id", async (c) => {
    ...
    return c.json(toListingDto(row));
  });
