...
export interface StoreRules {
  ...
  /** 库存推送策略（fl-monitor 消费）。 */
  inventory?: InventoryRules;
  /** 货源监控配置（fl-monitor 消费）。 */
...
export interface Listing {
  ...
  remoteVariantMap: Record<string, { variantId: string; inventoryItemId: string }> | null;
  /** 货源最后一次内容变化时间（fl-monitor 回写）。 */
